...
dataset_train = pd.read_csv('train/train_asm_label.csv')
X_train = dataset_train.iloc[:, 0:1039].values
print X_train
...
y_train = dataset_train.iloc[:, 1039:1040].values
print y_train.shape
print y_train
enc = OneHotEncoder()
enc.fit(y_train)  
...
train_x = train_x.reshape((train_x.shape[0], 1039, 1))
test_x = test_x.reshape((test_x.shape[0], 1039, 1))
# print train_x
# print train_y.shape
# print train_y

classifier.fit(train_x, train_y, batch_size = 10, epochs = 100, verbose=1)


print("Val Score: ", classifier.evaluate(test_x, test_y))

dataset_test = pd.read_csv('test/test_asm.csv')
X_test = dataset_test.iloc[:, 0:1039].values
X_test = StandardScaler().fit_transform(X_test)
X_test = X_test.reshape((X_test.shape[0], 1039, 1))
...
for i in ynew:
    result += [(i + 1)]
print result.reshape(result.shape[1], result.shape[0])
