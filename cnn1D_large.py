...
train_x = train_x.reshape((train_x.shape[0], 1804, 1))
test_x = test_x.reshape((test_x.shape[0], 1804, 1))
# print train_x
# print train_y.shape
# print train_y

classifier.fit(train_x, train_y, batch_size = 10, epochs = 3, verbose=1)


print("Val Score: ", classifier.evaluate(test_x, test_y))

dataset_test = pd.read_csv('test/LargeTest.csv')
X_test = dataset_test.iloc[:, 0:1804].values
X_test = StandardScaler().fit_transform(X_test)
X_test = X_test.reshape((X_test.shape[0], 1804, 1))
...
f_csv = open('test_result_large.csv', 'w')
csv_w = writer(f_csv)
